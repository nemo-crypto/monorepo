...
import { loadScript } from '@xatom/utils/src/script'
...
export default class ParticlesBackground extends React.Component {
  init() {
    ...
    if (id) {
      ...
      console.error('id is required for ParticlesBackground')
    }
  }
  componentDidMount() {
    const _this = this
    loadScript({
      src: window.publicPath + `libs/jqueryparticles/js/particles.min.js`,
      onload: function () {
        _this.init()
      }
    })
...
function initPugin(id) {
  window.particlesJS && window.particlesJS(id,
    {
      "particles": {
        "number": {
          "value": 160,//数量
